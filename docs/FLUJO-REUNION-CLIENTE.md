# 🎯 FLUJO DE REUNIÓN — PASO A PASO

**Para ejecutar con el cliente del SSTT (u otros similares).**

---

## PRE-REUNIÓN (preparación tuya)

- [ ] Llevas: Presentación Ejecutiva (impresa)
- [ ] Llevas: Kit de Diagnóstico (impresa o en phone)
- [ ] Llevas: Tabla de Valores Tipo (referencia personal)
- [ ] Llevas: Link a test MMA-OD (o app si está disponible)
- [ ] Datos del cliente: nombre, email, teléfono contacto

---

## MINUTO 1–3: PRESENTACIÓN (tú hablas)

**Objetivo:** Que entienda quiénes eres y qué ofreces (sin vendedor, sin hype).

**Guion:**

> "Hola. Soy Patricio, de CONSULTORAVIRTUAL. Hacemos un trabajo que es raro en Chile:
>
> Nos importa **ganar dinero con IA**, no tener IA. Eso significa: si tu proceso está roto, la tecnología solo lo multiplica. Primero ordenamos; después digitalizamos o automatizamos.
>
> **Esto es lo que hacemos:**
> - **Fase 1:** Diagnóstico (2–4 semanas, $450K, ).
> - **Fase 2:** Implementar lo que eligieron (Productividad, Digitalizar, Automatizar, o Anticipar).
>
> **Nuestras reglas:**
> - No necesitan personas técnicas nuevas — su equipo aprende.
> - Sin lock-in. Monthly, cancelable.
> - Resultados verificables, no promesas.
>
> Entonces: te propongo que hoy hagamos un test rápido (5 minutos), vemos dónde estás, y de eso sale si tiene sentido que hablemos de un diagnóstico."

**Material:** Presentación Ejecutiva

---

## MINUTO 4–9: TEST MMA-OD (5 preguntas, ustedes responden)

**Objetivo:** Identificar en qué estado están. **No juzgues las respuestas.**

**Usa:** Las 5 preguntas del Kit de Diagnóstico (Parte 1).

**Nota mientras responden.** Marca cuál sería el estado probable.

**Pregunta al final:**

> "¿Cuál de estos dolores es el más urgente?" (marca la respuesta de Pregunta 5)

---

## MINUTO 10–15: SÍNTESIS (tú analiza en vivo)

**Objetivo:** Mostrar que entendiste y que tienes un plan.

**Guion:**

> "Ojo, esto es un 'termómetro' rápido, no un diagnóstico. Pero veo que:
>
> **Hoy están en:** [ORDEN_SIN_DATOS / DATOS_SIN_ORDEN / EN_CONSTRUCCION — lo que sea]
>
> **Lo urgente es:** [El dolor que marcaron]
>
> **Para resolver eso, tienen dos caminos:**
>
> **Opción A - Diagnóstico (Fase 1):**  
> → 2–4 semanas, $450.000 CLP ( financia , ustedes )
> → Levanto el proceso completo, propongo 3–5 soluciones exactas, presupuestan las que quieran
> → Quedas con un documento que vale más que lo que pagaste.
>
> **Opción B - Audit rápida:**  
> → 1 semana, $150.000 CLP (ustedes pagan todo)
> → Fotografía el estado, propongo 3 opciones básicas
> → Menos detalle, pero más rápido.
>
> ¿Cuál te parece?"

---

## MINUTO 16–20: RESPUESTA DEL CLIENTE

### Si dice "Opción A" (Diagnóstico completo)

> "Excelente. Entonces:
>
> 1. Te dejo el contrato hoy (o mañana, como prefieras).
> 2. Me dan acceso a: zona de recepción, sistema actual, nombres de técnicos principales.
> 3. En 1 semana empiezo el levantamiento.
> 4. En 2–4 semanas tienen el reporte + propuestas.
>
> Mientras tanto, ¿necesitas que tramite lo de , o lo haces tú?"

**Material:** Referencia a "Modelo de Financiamiento " (si tienen uno)

---

### Si dice "Opción B" (Audit rápida)

> "Dale. Entonces:
>
> 1. Te dejo contrato hoy (1 página).
> 2. Voy 1 o 2 días a observar: recepción, reparación, entrega.
> 3. En 1 semana tienen reporte + 3 propuestas de Fase 2 con valores.
> 4. De ahí deciden si avanzan o no.
>
> ¿Cuándo podría venir a observar?"

---

### Si dice "Necesito pensarlo" / "Debo consultar al dueño"

> "Perfecto. Entonces:
>
> 1. Te dejo esta documentación (muestra: Presentación + Valores Tipo).
> 2. Si tienen preguntas, me escriben o me llaman.
> 3. Volvemos a hablar en [X días] con el dueño, si hace falta.
>
> ¿Para cuándo estaría bien que te llame de nuevo?"

**Material:** Presentación Ejecutiva + Valores Tipo (impresas)

---

### Si dice "Es caro" / "No tenemos presupuesto"

> "Entiendo. Pero ojo:
>
> - El diagnóstico:  paga 70%. Ustedes pagan .
> - Si el dinero es realmente cero hoy: podría hacerle un audit de $50K (bien acotado), solo fotografía.
> - O: volvemos a hablar en 3 meses cuando haya presupuesto.
>
> ¿Cuál de las tres?"

---

### Si dice "No, gracias. No nos interesa IA"

> "Dale, respeto. Una cosa: si en 6 meses la logística sigue siendo un problema, me escriben. Capaz para ese momento tiene más sentido."

**Don't be pushy.**

---

## MINUTO 20+: CIERRE ADMINISTRATIVO

### Si aceptaron (Opción A o B)

**Preguntar:**

- [ ] Nombre completo del contacto principal
- [ ] Email y teléfono
- [ ] ¿Quién es el que decide? ¿Dueño, gerente? ¿Necesito reunirme con él?
- [ ] ¿Cuándo puedo empezar? (Fase 1, 1–2 semanas) / ¿Cuándo puedo ir a observar? (Audit, esta semana)
- [ ] ¿Acceso al sistema actual, si tienen? (nombre del sistema, credenciales si me autorizan)

**Dejar:**

- [ ] Contrato Fase 1 o Audit firmado (ambos)
- [ ] Documentación impresa (Presentación Ejecutiva + Valores Tipo)
- [ ] Tu email y teléfono para dudas

---

## DOCUMENTOS PARA LLEVAR

### Impresión de bolsillo

1. **PRESENTACION-EJECUTIVA.md** (1 página, frente-dorso)
2. **VALORES-TIPO-COTIZACION.md** (1 página con matriz de casos)
3. **KIT-DIAGNOSTICO-SSTT.md** (para referencia tuya durante reunión)

### En el phone

- Test MMA-OD (si es app) o link a sistemaaiprocess

### En la carpeta

- Contrato Fase 1 (en blanco, para llenar)
- Contrato Fase 2 (template, solo para referencia)

---

## SEGUIMIENTO POST-REUNIÓN

### Si aceptaron (24h después)

**Email al cliente:**

> "Hola [Nombre],
>
> Gracias por la reunión de hoy. Adjunto confirmamos:
> - Fase 1 (Diagnóstico): $450.000 CLP 
> - Duración: 2–4 semanas
> - Inicio: [Fecha que acordamos]
>
> Para tramitar lo de , deberás contactar a [info si la tienes].
> Yo arranco cuando tengas confirmado que  entra.
>
> Cualquier duda, me escribes.
>
> Saludos,  
> Patricio"

---

### Si dijeron "Necesito pensarlo"

**Email a los 3 días:**

> "Hola [Nombre],
>
> ¿Cómo va? ¿Tuviste chance de mirar la documentación?
>
> Si tenés preguntas o querés que nos reunamos con el dueño, avísame.
>
> Saludos,  
> Patricio"

---

### Si dijeron "No"

**No insistas.** Pero:

**Email a los 6 meses:**

> "Hola [Nombre],
>
> Pasó un tiempo. ¿Cómo anda la logística? Si en algún momento ven que la IA puede ayudar, me buscan.
>
> Saludos,  
> Patricio"

---

## ERRORES TÍPICOS A EVITAR

❌ **No hagas:**

- Prometer resultados ("80% más rápido") sin Fase 1
- Aceptar hacer un proyecto sin Fase 1
- Comprometerse a precio fijo sin Ver el sistema
- Vender Automatizar sin diseñar quién revisa
- Argumentar mucho contra objeciones (escucha, no vendas)

✅ **Sí hacé:**

- Escuchá más de lo que hablás
- Haz el test MMA-OD, no adivines el estado
- Propone Fase 1 como puerta de entrada
- Sé claro con los valores tipo (no son exactos, son referencias)
- Deja que piensen

---

## CHECKLIST FINAL ANTES DE IRTE

- [ ] Contrato firmado (ambos)
- [ ] Email intercambiado
- [ ] Teléfono del contacto técnico (si tienen)
- [ ] Aclarado cuándo empiezo (Fase 1) o cuándo voy a observar (Audit)
- [ ] Preguntado sobre financiamiento 

---

*Versión: 2026-07-30*  
*Casos de uso: SSTT, Pymes con desorden operativo*


