# 📝 Notas de Reunión — SSTT Ernesto Andino

**Fecha reunión:** [A llenar]  
**Participantes:** [A llenar]  
**Lugar:** [A llenar]

---

## Test MMA-OD (Respuestas)

### Pregunta 1: ¿El proceso de recepción está documentado?
- [ ] No; cada técnico lo hace diferente
- [ ] Sí, pero solo en la cabeza del dueño
- [ ] Sí, escrito pero nadie lo sigue
- [ ] Sí, escrito y se sigue

**Observación:** [A llenar]

---

### Pregunta 2: ¿Dónde anotas que llegó un teléfono?
- [ ] Cuaderno; lista de WhatsApp; correo
- [ ] Planilla Excel (manual)
- [ ] Sistema / software (sin integraciones)
- [ ] Sistema integrado con el resto del flujo

**Observación:** [A llenar]

---

### Pregunta 3: ¿Cómo sabe el cliente cuándo sale su teléfono?
- [ ] Llama y pregunta
- [ ] Tú le avisas (cuando te acordás)
- [ ] Mensaje automático cuando está listo
- [ ] Portal donde ve el estado en tiempo real

**Observación:** [A llenar]

---

### Pregunta 4: ¿Cuánto tarda en promedio reparar + limpiar + probar un teléfono?
- [ ] No sabemos; depende del modelo
- [ ] Sabemos el promedio general (~3 días)
- [ ] Sabemos por modelo (iPhone X: 2 días, Samsung: 3 días)
- [ ] Predecimos con 90%+ de precisión cuándo termina cada uno

**Observación:** [A llenar]

---

### Pregunta 5: ¿Qué duele más hoy?
- [ ] Clientes llaman diciendo "dónde está mi teléfono"
- [ ] Se pierden teléfonos en la recepción
- [ ] No sé cuántos están en cola, cuántos en reparación, cuántos listos
- [ ] Tarifa de reparación vs tiempo real de trabajo no cierra

**Observación:** [A llenar]

---

## Estado MMA-OD Identificado

**Estado:** [A llenar]

**Justificación:** [A llenar]

---

## Información Operativa Levantada

### Recepción y Registro
- Volumen por semana: [A llenar] teléfonos
- Modelos principales: [A llenar]
- Cómo registran hoy: [A llenar]
- Datos que anotan: [A llenar]
- Hay fotografía de entrada: [ ] Sí [ ] No [ ] A veces

### Reparación
- Cantidad de técnicos: [A llenar]
- Tiempo promedio: [A llenar]
- Técnico más rápido: [A llenar] (tiempo: [A llenar])
- Cuello de botella identificado: [A llenar]
- Fallas más comunes: [A llenar]

### Entrega
- Cómo notifican al cliente: [A llenar]
- Existe listado "listos para retirar": [ ] Sí [ ] No
- Dónde se guardan: [A llenar]
- Consultas diarias sobre dónde está: [A llenar] (estimado)

### Reportes Actuales
- ¿Tienen reporte? [ ] Sí [ ] No
- Frecuencia: [A llenar]
- Quién lo arma: [A llenar]
- Qué preguntas hace el gerente: [A llenar]

---

## Decisión Comercial

**Cliente elegió:**
- [ ] Opción A: Fase 1 (Diagnóstico)
- [ ] Opción B: Audit rápida ($150K)
- [ ] Pendiente decisión (vuelvo a llamar)
- [ ] Cliente dice No

**Si dice SÍ a Fase 1:**
- Fecha inicio: [A llenar]
- Contacto técnico: [A llenar]
- Email contacto: [A llenar]
- Permiso para acceder a: [ ] Recepción [ ] Reparación [ ] Sistema [ ] Datos históricos
- Financiamiento Sercotec: [ ] Lo tramita cliente [ ] Lo tramitamos nosotros

**Si dice NO o "Pensemos":**
- Motivo: [A llenar]
- Próximo contacto: [A llenar]
- Observaciones: [A llenar]

---

## Documentos Entregados

- [ ] Presentación Ejecutiva (impresa)
- [ ] Kit de Diagnóstico (impresa o digital)
- [ ] Tabla de Valores (impresa)
- [ ] Cotización Borrador (digital)
- [ ] Contrato Confidencialidad (para firmar)

---

## Impresiones Generales

**Nivel de interés:** [ ] Alto [ ] Medio [ ] Bajo

**Facilidad de acceso:** [ ] Alta [ ] Media [ ] Baja

**Receptividad a IA:** [ ] Alto [ ] Medio [ ] Bajo

**Observaciones finales:** [A llenar]

---

## Próximos Pasos

1. [Acción 1 - Responsable - Plazo]
2. [Acción 2 - Responsable - Plazo]
3. [Acción 3 - Responsable - Plazo]

---

**Fecha actualización:** [A llenar]  
**Actualizado por:** [A llenar]
