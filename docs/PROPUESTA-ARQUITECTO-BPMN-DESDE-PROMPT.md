...
- **`sistemaaiprocess`** (producto hermano, mismo dueño): Next.js 16 + React 19 + Prisma + Auth.js v5 con Google OAuth (`src/auth.ts`) + Upstash rate-limit. **Corrección 2026-08-05 (verificada con DNS real, `aiprocess.cl` resuelve a IPs de Vercel):** la app en sí corre en **Vercel**, no self-hosted — la afirmación original de esta línea ("self-hosted en VPS/EasyPanel, no Vercel") estaba equivocada, confundía el hosting de la app con el de la base de datos. Lo que sí es self-hosted en el VPS/EasyPanel de Patricio es únicamente **Postgres** (confirmado con consultas reales contra la base, ver `docs/referencia/MERCADO-PAGO.md`/bitácora 2026-08-04) — Vercel se conecta a esa base externa. **No tiene Stripe ni ningún flujo de pago** — solo login.
...
| Hosting | ~~VPS/EasyPanel~~ → **Vercel** (corrección 2026-08-05, ver nota al pie de "Estado Actual" — `sistemaaiprocess` corre en Vercel, no self-hosted; solo Postgres vive en el VPS/EasyPanel) | Mismo patrón que `sistemaaiprocess` (Vercel + Postgres externo), cero curva de aprendizaje nueva. |
