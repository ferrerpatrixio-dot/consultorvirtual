...
| 5. Alcance de exportación v1 | **También exportar a `.bpmn` XML real**, no solo PNG/PDF. **Impacto:** esto reintroduce la necesidad de mapear el esquema plano del prototipo a un BPMN 2.0 válido (lanes, tipos de nodo, secuencia) — justifica retomar parcialmente el esquema de la skill `bpmn-architect` (no los campos de resiliencia, pero sí la estructura `nodes`/`edges` con tipos BPMN estándar). **El tamaño total del proyecto sube de M a L.** |
